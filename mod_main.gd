...
var _mod_version: String = "unknown"


func _init() -> void:
    _mod_version = _get_version_from_manifest()
...
func _ready() -> void:
    ...
    ModLoaderLog.info("Ready - v%s" % _mod_version, MOD_NAME)


func _get_version_from_manifest() -> String:
    var manifest_path: String = get_script().resource_path.get_base_dir() + "/manifest.json"
    var file: FileAccess = FileAccess.open(manifest_path, FileAccess.READ)
    if not file:
        return "unknown"
    var json = JSON.parse_string(file.get_as_text())
    if json and json is Dictionary:
        return json.get("version_number", "unknown")
    return "unknown"
...
func _attempt_smart_connection(source: ResourceContainer, source_type: int, target_window: WindowBase) -> void:
    ...
    # Note: _find_compatible_container only returns empty input slots, so no need to
    # delete existing connections - if all slots are full, no connection is created
...
func _can_connect(source: ResourceContainer, target: ResourceContainer, source_type: int) -> bool:
    ...
    if source_type == Utils.connections_types.OUTPUT:
        ...
        if source.get_connector_color() == "black" or target.get_connector_color() == "black":
            return false
        # Only allow connections to empty input slots (don't replace existing connections)
